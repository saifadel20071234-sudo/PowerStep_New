...
class PiezoSerialWorker(threading.Thread):
    ...
    def _read_loop(self, ser) -> None:
        while not self.stop_event.is_set():
            try:
                # تفريغ الذاكرة المؤقتة (Backlog) لو البايثون أبطأ من الأردوينو
                # هذا يمنع الديلاي الطويل جداً الذي يتراكم مع الوقت
            ...
            try:
                ...
                # Process WiFi data (if present from the combined ESP8266 script)
                if "people_count" in raw or "device_count_raw" in raw:
                    wifi_record = self.engine.process_wifi_reading(raw)
                    self.db.insert_wifi_reading(wifi_record)
                    self.alerts.evaluate_wifi(wifi_record)
                    get_bridge().on_wifi(wifi_record)
                    self.liveness.mark("wifi")
                    
            except Exception:
                logger.exception("Failed to process a piezo/combined reading.")
