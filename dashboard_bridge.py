...
NUM_TILES = 16
PRESS_WINDOW_SEC = 1.0        # المدة اللي بتفضل فيها البلاطة مضيئة بعد الضغط
BROADCAST_INTERVAL_SEC = 0.2  # بث الداش بورد 5 مرات في الثانية لتقليل الديلاي
...
class DashboardBridge:
    ...
    def on_piezo(self, record) -> None:
        with self._lock:
            ...
            now = time.time()
            new_gen = _pick(record, "generation_w", "power_w", default=0.0)
            
            # --- Peak Hold Logic (للحفاظ على ثبات الأرقام في الداش بورد) ---
            if new_gen > 0.1:
                self._last_piezo = record
                self._last_peak_time = now
            else:
                # إذا كانت القراءة صفر، نحتفظ بالقراءة العالية لمدة ثانيتين حتى لا ترمش الشاشة
                if hasattr(self, '_last_peak_time') and now - self._last_peak_time < 2.0:
                    # نحدث فقط القيم التراكمية وعداد الأشخاص
                    if "cumulative_gen_wh" in record:
                        self._last_piezo["cumulative_gen_wh"] = record["cumulative_gen_wh"]
                    if "footfall" in record:
                        self._last_piezo["footfall"] = record["footfall"]
                else:
                    self._last_piezo = record
            ...
            pressed = tid is not None and (
                ...
                ai_detected or raw_footfall or new_gen > 0.1
            )
            if pressed:
    ...
    def _snapshot(self) -> dict:
        ...
        # --- محاكاة الاستهلاك للداش بورد (لتشغيل عداد الاكتفاء الذاتي) ---
        con_w = 5.0  # استهلاك وهمي 5 واط للمصابيح
        if gen_w > 0.1:
            # إذا كان هناك توليد قوي، نجعل الاكتفاء يرتفع
            self_suff = min(100.0, (gen_w / con_w) * 100.0)
        else:
            self_suff = 0.0
        ...
        # ربط الـ Footfall بالضغط على البيزو مباشرة (تزيد مع الضغط وتصبح صفر بدونه)
        if len(active_steps) > 0:
            footfall = len(active_steps) * 3
        else:
            footfall = 0
